...
TEST(AIFollowRouteAction, ValidRouteAndMovementAgentSubmitsRunningTask)
{
    ...
    EXPECT_EQ(
        AIFollowRouteAction::Start(aiSystem, world, agent, MakeFollowRoute({5.0f, 0.0f, 0.0f})),
...
TEST(AIFollowRouteAction, InvalidRouteDoesNotReplaceExistingTask)
{
    ...
    EXPECT_EQ(
        AIFollowRouteAction::Start(aiSystem, world, agent, std::move(invalidRoute)),
...
TEST(AIFollowRouteAction, MissingMovementComponentsDoNotReplaceExistingTask)
{
    ...
    EXPECT_EQ(
        AIFollowRouteAction::Start(aiSystem, world, agent, MakeFollowRoute({0.0f, 0.0f, 5.0f})),
        AIActionExecutionStatus::Failed);
    EXPECT_EQ(aiSystem.GetActionStatus(agent), AIActionExecutionStatus::Running);
}
...
TEST(AIFollowRouteAction, NonAIEntityDoesNotSubmitTask)
{
    ...
    EXPECT_EQ(
        AIFollowRouteAction::Start(aiSystem, world, nonAIEntity, MakeFollowRoute({0.0f, 0.0f, 5.0f})),
        AIActionExecutionStatus::Failed);
...
TEST(AIFollowRouteAction, OnePointRouteCompletesSuccessfully)
{
    ...
    EXPECT_EQ(
        AIFollowRouteAction::Start(aiSystem, world, agent, MakeOnePointRoute({0.0f, 0.0f, 0.0f})),
...
// Protects existing traversal behavior: unsupported traversal links remain a
// runtime tick failure rather than being handled by the action starter.
TEST(AIFollowRouteAction, TraversalRequirementFailsDuringTick)
{
    GameplayWorld world{};
    AISystem aiSystem{};
    const EntityHandle agent = CreateFollowRouteAgent(world);
    ...
    ASSERT_EQ(
        AIFollowRouteAction::Start(aiSystem, world, agent, MakeTraversalRoute()),
        AIActionExecutionStatus::Running);
