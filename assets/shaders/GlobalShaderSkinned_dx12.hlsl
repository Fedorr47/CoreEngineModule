...
cbuffer PerBatchCB : register(b0)
{
    ...
    // Forward uses spec/gloss fallback through these bindless indices, while deferred
...
struct VSOut
{
	...
	float4 shadowPos : TEXCOORD5; // directional shadow clip (legacy / not used for CSM)
#if defined(CORE_PLANAR_CLIP) && CORE_PLANAR_CLIP
...
VSOut VSMain(VSIn IN)
{
    ...
    float4 world = mul(float4(skinnedPos, 1.0f), uModel);

    float3x3 normalMatrix = InverseTranspose3x3(model3x3);
    float3 nrmW = normalize(mul(skinnedNrm, normalMatrix));
    float3 tangentW = normalize(mul(skinnedTangent.xyz, model3x3));
    tangentW = normalize(tangentW - nrmW * dot(nrmW, tangentW));
    float3 bitangentW = normalize(cross(nrmW, tangentW)) * skinnedTangent.w;
