...
export namespace rendern
{
    ...
    {
        EditorSelectedObjectKind kind{ EditorSelectedObjectKind::None };
        int index{ -1 };

        [[nodiscard]] bool IsValid() const noexcept
        {
            return kind != EditorSelectedObjectKind::None && index >= 0;
        }

        [[nodiscard]] friend bool operator==(const EditorSelectedObjectId& lhs,
    ...
    // panels do not conflate gameplay-controlled runtime focus with editor
    ...
    public:
        ...
        void SetSelectedObject(const EditorSelectedObjectId objectId)
        {
            ClearSelection();
            if (!objectId.IsValid())
            {
        ...
        {
            for (const int selectedNode : selectedSceneNodes_)
            {
    ...
    private:
        ...
        EntityHandle observedRuntimeEntity_{ kNullEntity };
    };
}
